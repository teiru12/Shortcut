	            ...
	            <div class="col-lg-12 col-md-12">
	                <div class="card">
	                	<div class="card-header">
	                    	<div style="text-align:center"><h5>신고게시판</h5></div>
	                    </div>
	                    <div class="card-content">
	                    	<c:if test="${ id != null}">
	                    		<a href="reportWriteForm.cut" class="btn btn-outline-primary" style="float: right; margin-right: 25px;">글쓰기</a>
	                    	</c:if>
	                        <div class="card-body">
                        		<table class="table">
	                        		<tr>
									...
									<c:if test = "${empty noticeTopList and empty reportList}">
										<tr>
		                        	...
		                        	<c:forEach var="reportItem" items="${reportList}">
										<c:choose>
											<%-- 삭제되지 않은 게시글 --%>		
	                               			<c:when test="${reportItem.ISDEL == 'N'}">
			                        			<tr>
			                               			<td class="list-group-horizontal-sm">${reportItem.REPORTIDX }</td>
			                               			<td class="list-group-horizontal-sm"><a href="reportDetail.cut?REPORTIDX=${reportItem.REPORTIDX}">${reportItem.TITLE }</a></td>
			                               			<td class="list-group-horizontal-sm">
														...
														<div class="dropdown-menu">
															<button class="btn btn-icon-default" onClick="writerDetail('${reportItem.ID}')">회원 정보</button><br>
															<c:if test="${! empty sessionScope.id}">
																<button type="button" class="openModal btn btn-icon-default" data-bs-toggle="modal"
																data-bs-target="#inlineForm"
																data-s="<%= request.getSession().getAttribute("id") %>" data-g="${reportItem.ID}">
																	쪽지 보내기
																</button><br>
			                               			...
			                               			<td class="list-group-horizontal-sm">-</td>
			                               			<td class="list-group-horizontal-sm">-</td>
			                               			<td class="list-group-horizontal-sm"><fmt:formatDate pattern="yyyy-MM-dd" value="${reportItem.REPORTDATE }"/></td>
			                               		</tr>
			                               	</c:when>
