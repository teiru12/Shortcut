	            ...
	            <div class="col-lg-12 col-md-12">
	                <div class="card">
	                	<div class="card-header">
	                    	<div style="text-align:center"><h5>뉴스게시판</h5></div>
	                    </div>
	                    <div class="card-content">
	                    <!-- 회원만 작성하기 버튼 활성화 -->
							<c:if test="${! empty sessionScope.id}">
	                    	<a href="newsWriteForm.cut" class="btn btn-outline-primary" style="float: right; margin-right: 25px;">글쓰기</a>
	                    	</c:if>
	                        <div class="card-body">
                        		<table class="table">
	                        		<tr>
									...
									<c:if test = "${empty noticeTopList and empty newsList}">
										<tr>
		                        	...
		                        	<c:forEach var="newsItem" items="${newsList}">
										<c:choose>
											<%-- 삭제되지 않은 게시글 --%>		
	                               			<c:when test="${newsItem.ISDEL == 'N'}">
			                        			<tr>
			                               			<td class="list-group-horizontal-sm">${newsItem.NEWSIDX }</td>
			                               			<td class="list-group-horizontal-sm"><a href="newsDetail.cut?NEWSIDX=${newsItem.NEWSIDX}">${newsItem.TITLE }</a></td>
			                               			<td class="list-group-horizontal-sm">
														...
														<div class="dropdown-menu">
															<button class="btn btn-icon-default" onClick="writerDetail('${newsItem.ID}')">회원 정보</button><br>
															<c:if test="${! empty sessionScope.id}">
																<button type="button" class="openModal btn btn-icon-default" data-bs-toggle="modal"
																data-bs-target="#inlineForm"
																data-s="<%= request.getSession().getAttribute("id") %>" data-g="${newsItem.ID}">
																	쪽지 보내기
																</button><br>
			                               			...
			                               			<td class="list-group-horizontal-sm">${newsItem.READCOUNT }</td>
			                               			<td class="list-group-horizontal-sm">${newsItem.GOOD }</td>
			                               			<td class="list-group-horizontal-sm">${newsItem.BAD }</td>
			                               			<td class="list-group-horizontal-sm"><fmt:formatDate pattern="yyyy-MM-dd" value="${newsItem.NEWSDATE }"/></td>
			                               		</tr>
			                               	</c:when>
